...
# ─────────────────────────────────────────────────────────────────────────────
# 3. DATA AUGMENTATION  (Prevents Feature Collapse)
# ─────────────────────────────────────────────────────────────────────────────
def augment_telemetry_features(
    ...
    """Synthesize varied telemetry features for all batches using Gaussian noise.
    ...
    Strategy:
      ...
      2. For each of the other 59 batches, add Gaussian noise:
             augmented = baseline × (1 + N(0, σ))
         where σ = `noise_fraction` (default 8 %).
      3. Optionally correlate noise with production settings for realism:
         batches with higher Drying_Temp get proportionally higher
         drying-phase Temperature features.
    ...
    noise_fraction : float
        Standard deviation of Gaussian noise as fraction of feature magnitude.
    seed : int
    ...
    # ── Generate noise matrix (n_batches × n_features) ──────────────────
    noise = rng.normal(loc=0.0, scale=noise_fraction, size=(n_batches, len(feature_cols)))

    # Multiplicative noise: augmented = baseline × (1 + noise)
    augmented_matrix = baseline_values[np.newaxis, :] * (1.0 + noise)

    # ── Correlate with production settings for realism ──────────────────
    # Batches with higher Drying_Temp -> inflate Drying-phase Temperature features
    if "Drying_Temp" in production_df.columns:
        drying_temp_vals = production_df["Drying_Temp"].values.astype(float)
        drying_temp_median = np.median(drying_temp_vals)
    ...
    # Batches with higher Machine_Speed -> inflate Compression-phase motor/vibration
    if "Machine_Speed" in production_df.columns:
        speed_vals = production_df["Machine_Speed"].values.astype(float)
        speed_median = np.median(speed_vals)
    ...
    # Batches with higher Compression_Force -> inflate compression-related forces
    if "Compression_Force" in production_df.columns:
        force_vals = production_df["Compression_Force"].values.astype(float)
        force_median = np.median(force_vals)
    ...
    # ── Ensure non-negative values (sensor readings can't be negative) ──
    ...
    print(f"[DATA LAYER] Augmented telemetry features for {n_batches} batches "
          f"(σ = {noise_fraction * 100:.0f}%, correlated with production settings)")
